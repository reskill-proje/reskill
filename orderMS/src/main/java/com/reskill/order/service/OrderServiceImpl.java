...
import com.reskill.order.repository.ProductsOrderedRepository;
import com.reskill.order.utility.NoPk;
import com.reskill.order.utility.OrderMessage;
import com.reskill.order.validator.OrderValidator;
...
public class OrderServiceImpl implements OrderService {
	...
	@Override
	public OrderedPlacedDTO placeOrder(List<ProductDTO> productList, List<CartDTO> cartList, OrderDTO orderDTO) throws OrderException {
		...
		order.setStatus(OrderMessage.ORDER_PLACED.toString());	
		order.setAmount(0f);
		...
		for(int i = 0; i<cartList.size();i++) {
			...
			prodO.setNoPrimaryKeys(new NoPk(cartList.get(i).getBuyerId(),productList.get(i).getProdId()));
			prodO.setQuantity(cartList.get(i).getQuantity());
			productsOrdered.add(prodO);				
		}		
		productsOrderedRepository.saveAll(productsOrdered);
		orderRepository.save(order);
		OrderedPlacedDTO orderPlaced = new OrderedPlacedDTO();
